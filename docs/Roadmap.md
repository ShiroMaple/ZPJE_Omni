...
- [ ] **应用分类与部门标签系统**
  - [ ] `App` 模型新增 `mainDeptId` 字段（外键关联 `departments.id`），表示应用所属部门
  - [ ] 设定默认值语义为"未分类"（`mainDeptId = null` 时视为未分类）
  - [ ] 管理后台维护应用时，`mainDeptId` 为必填项，选取控件按 **单位 → 部门** 层级展示
- [ ] **管理后台 UI（Admin Panel）**
  - [ ] 新增 `/admin` 路由（仅管理员可访问）
  - [ ] 实现应用列表页：展示所有已注册应用，支持搜索/排序
  - [ ] 实现应用编辑表单：名称、图标、URL、简介、分类、所属部门、维护模式开关、排序权值
  - [ ] 实现应用新增 / 删除功能
  - [ ] 实现对应的 CRUD API 路由（`/api/admin/apps`）

### 1.2 门户左侧部门切片器（Sidebar Filter）

> 在门户左侧设置侧边栏，以部门维度对应用进行筛选。

- [ ] 实现左侧侧边栏组件（`Sidebar.tsx`）
- [ ] 查询所有 `mainDeptId` 不为空、且名下 app 数量 > 0 的部门列表
- [ ] 侧边栏展示这些部门名称，点击后筛选出对应部门下的应用卡片
- [ ] 提供"全部应用"选项清除筛选
- [ ] 响应式布局：移动端侧边栏可折叠
...
- [ ] **后端健康检测服务**
  - [ ] 新增 `/api/cron/health-check` 路由（或独立脚本），定时（如每 30s）向各子系统 URL 发起 HTTP HEAD 探活
  - [ ] 根据响应状态码更新 `healthStatus`：`200` → `HEALTHY`，`>500ms` → `SLOW`，`5xx/超时` → `UNHEALTHY`
  - [ ] 更新 `lastCheckedAt` 时间戳
- [ ] **前端状态可视化**
  - [ ] 应用卡片上根据 `healthStatus` 显示状态指示灯：🟢 运行中 / 🟡 响应迟缓 / 🔴 服务异常
  - [ ] 服务异常时，点击卡片弹出友好提示弹窗（"该系统当前维护中，请稍后再试"），而非直接跳转
...
- [ ] 管理后台增加维护模式开关按钮
- [ ] 前端卡片在 `isMaintenance=true` 时显示"维护中"标识，禁止点击跳转
- [ ] 维护模式下卡片样式变为灰色/半透明
...
- [ ] **前端退出按钮**
  - [ ] 在 `Dashboard.tsx` Header 的用户 Profile 区域添加"退出登录"按钮
  - [ ] 点击后调用 `POST /api/auth/logout`，成功后刷新页面或跳转至登录引导页
- [ ] **会话超时自动登出**
  - [ ] JWT 过期后（8 小时），前端检测到 401/无效 token 时自动触发登出流程
  - [ ] 超时登出同样通知致远 OA 释放会话
...
```text
【第一阶段：基础运维与扩展】 ← 当前重点
  ├── 1.1 动态应用注册中心 + 管理后台          ██████░░░░ 60% (模型/渲染已完成，后台 UI 与部门标签待开发)
  ├── 1.2 门户左侧部门切片器                   ░░░░░░░░░░ 0%
  ├── 1.3 子应用健康探活                        ██░░░░░░░░ 20% (模型字段已预留)
  ├── 1.4 应用维护模式                          ██░░░░░░░░ 20% (模型字段已预留)
  ├── 1.5 完整单点登出 (SLO)                    ██████░░░░ 60% (后端 API 已完成，前端按钮待实现)
  └── 1.6 明暗模式切换                          ░░░░░░░░░░ 0%
